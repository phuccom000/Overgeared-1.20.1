 ...
 * RecipeBookType.create("FORGING_ANVIL");
 * private final RecipeType<ForgingRecipe> recipeType =
 * ForgingRecipe.Type.INSTANCE;
 * private final CraftingContainer craftSlots = new
 ...
 * }
 * 
 * public SmithingAnvilMenu2(int pContainerId, Inventory inv, FriendlyByteBuf
 * extraData) {
 * this(pContainerId, inv,
 * inv.player.level().getBlockEntity(extraData.readBlockPos()), new
 * SimpleContainerData(11));
 * }
 * 
 * 
 * public SmithingAnvilMenu2(int pContainerId, Inventory inv, BlockEntity
 * entity, ContainerData data) {
 ...
 * for (int i = 0; i < 3; ++i) {
 * for (int j = 0; j < 3; ++j) {
 * this.addSlot(new SlotItemHandler(iItemHandler, j + i * 3, 30 + j * 18, 17 + i
 * * 18));
 * }
 * }
 * //output slot
 ...
 *//*
    ...
    * this.addSlot(new Slot(this.craftSlots, col + row * 3, 30 + col * 18, 17 + row
    * * 18));
    * }
    * }
    * this.addSlot(new ResultSlot(inv.player, this.craftSlots, this.resultSlots, 0,
    * 124, 35));
    * 
    ...
    * slotChangedForgingGrid(this, this.level, this.player, craftSlots,
    * resultSlots);
    * }
    * super.slotsChanged(inventoryIn);
    * }
    * 
    * protected static void slotChangedForgingGrid(AbstractContainerMenu pMenu,
    * Level pLevel, Player pPlayer, CraftingContainer pContainer, ResultContainer
    * pResult) {
    * if (!pLevel.isClientSide) {
    * ServerPlayer serverplayer = (ServerPlayer) pPlayer;
    * ItemStack itemstack = ItemStack.EMPTY;
    * Optional<ForgingRecipe> optional =
    * pLevel.getServer().getRecipeManager().getRecipeFor(ForgingRecipe.Type.
    * INSTANCE, pContainer, pLevel);
    * if (optional.isPresent()) {
    * ForgingRecipe forgingRecipe = optional.get();
    * if (pResult.setRecipeUsed(pLevel, serverplayer, forgingRecipe)) {
    * ItemStack itemstack1 = forgingRecipe.assemble(pContainer,
    ...
    * serverplayer.connection.send(new
    * ClientboundContainerSetSlotPacket(pMenu.containerId,
    ...
    * private static final int TE_INVENTORY_SLOT_COUNT = 11; // must be the number
    * of slots you have!
    * 
    * @Override
    ...
    * 
    * return maxProgress != 0 && progress != 0 ? progress * progressArrowSize /
    * maxProgress : 0;
    * }
    * 
    * @Override
    ...
    * for (int i = 0; i < 3; ++i) {
    * for (int l = 0; l < 9; ++l) {
    * this.addSlot(new Slot(playerInventory, l + i * 9 + 9, 8 + l * 18, 84 + i *
    * 18));
    * }
    * }
    * }
